...
import { Badge, HStack, Text, Avatar, AvatarGroup } from '@chakra-ui/react'
...
const Home = () => {
  ...
  const handleDragEnd = (event) => {
    ...
    const card = columns[from].find((item) => item.id === activeId)
    ...
    setColumns((prev) => {
      const updated = { ...prev }
      updated[from] = updated[from].filter((item) => item.id !== activeId)
      updated[over.id] = [...updated[over.id], card]
      return updated
    })
  ...
  const addCard = () => {
    ...
    const firstColumnKey = Object.keys(columns)[0] // 'To Do'

    setColumns((prev) => ({
      ...prev,
      [firstColumnKey]: [...prev[firstColumnKey], card],
    }))

    setNewCard({ title: '', description: '', priority: '' })
  }
      ...
      <div className='homeHeader'>
        ...
        <div className='homeContainer'>
          ...
          <div className='homeContent'>
            ...
            >
              <HStack gap={'1rem'}>
                <Text fontWeight={'600'} fontSize={'2.5rem'} color={'#0D062D'}>
                  Mobile App
                </Text>
                <HStack gap={'1rem'}>
